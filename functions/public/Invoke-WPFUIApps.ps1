function Invoke-WPFUIApps {
    ...
    # Iterate through 'entryInfo' by panel, category, and application
    $count = 0
    foreach ($panelKey in ($entryInfo.Keys | Sort-Object)) {
        ...
        foreach ($category in ($entryInfo[$panelKey].Keys | Sort-Object)) {
            $count++
            ...
            $entries = $entryInfo[$panelKey][$category] | Sort-Object Order, Name
